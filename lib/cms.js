...
export async function listAllPostsForAdmin() {
  ...
  await ensureBlogTables(db);
  const result = await db.execute("SELECT * FROM posts ORDER BY updated_at DESC");
  return result.rows.map(fromPostRow);
}

export async function deletePost(slug, { actor = "admin" } = {}) {
  const db = await ensureBlogTables();
  const existing = await db.execute({ sql: "SELECT payload FROM posts WHERE slug = ? LIMIT 1", args: [slug] });
  if (!existing.rows[0]) {
    const err = new Error("글을 찾지 못했습니다.");
    err.status = 404;
    throw err;
  }
  // 삭제 전 마지막 payload를 리비전으로 백업해 복구 가능하게 남긴다
  await db.execute({
    sql: "INSERT INTO post_revisions (slug, payload, created_at, created_by) VALUES (?, ?, ?, ?)",
    args: [slug, existing.rows[0].payload, new Date().toISOString(), `${actor}:delete`],
  });
...
export function validatePostPayload(raw, expectedSlug = "") {
  ...
  // 마크다운으로 작성된 글은 저장 시 blocks로 변환한다 (에디터의 마크다운 모드)
  if (source.markdownIsSource && typeof source.contentMarkdown === "string" && source.contentMarkdown.trim()) {
    source.content = { ...(source.content || {}), blocks: markdownToBlocks(source.contentMarkdown) };
  }
  ...
  const payload = normalizePostPayload({
    ...
    writer: source.author || source.writer || DEFAULT_AUTHOR,
