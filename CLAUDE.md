...
- 브랜드 표기: **System Web**, 로고는 `/logo.png`. 사업자(법적 주체)는 **consolve** — 푸터 저작권은 "© 2026 consolve · System Web"로 통일. "Visible Dev"는 폐기된 옛 이름이므로 어디에도 남기지 말 것.
- 핵심 메시지: "시스템으로 만드는, 빠르고 퀄리티 있는 웹사이트·자사몰" / **5분 견적**.
- 가격: **견적 엔진 v2 단가표와 동일 기준** (consolve-quote-automation `config/price-catalog.yaml`, 2026-07 대표 확정).
  기본 20만(랜딩 1p / 홈 5p / Cafe24 셋업) + 디자인 레퍼런스 10·시안 구현 30·제작 50 + 백엔드 폼알림 30·저장관리자 60·외부API 100~ + Cafe24 화면 모듈 20/개 + 추가 페이지 5만/1p + 3주 이내 일정 +15%.
  /price 카드는 대표 구성 기준: 랜딩 30만~ / 홈 80만~ / 자사몰 90만~. 단가를 바꿀 땐 견적 엔진 카탈로그와 반드시 함께 수정한다.
