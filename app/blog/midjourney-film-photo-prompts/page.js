...
export default async function MidjourneyFilmPhotoPromptsPage() {
  ...
  const heroImage = article.hero?.image || article.hero?.src || "";
  const heroStyle = heroImage ? { backgroundImage: `url("${heroImage}")` } : undefined;
  return (
    <>
      ...
      <main className="article-wrap" data-content-source={dbPost ? "db-json" : "static-fallback"}>
        ...
        <div className={heroImage ? "hero has-image" : "hero"} style={heroStyle} role="img" aria-label={article.hero?.alt || article.heroLabel || "블로그 대표 이미지"} />
