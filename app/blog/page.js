  ...
  return (
    <main className="blog-shell">
      <header className="blog-hero">
        ...
        <p>
          seo-blog CRM이 shared DB에 저장한 공개 콘텐츠를 바로 보여줍니다. Google 노출을 먼저 만들고, AI 검색이 인용하기 쉬운 구조와 네이버 발견성을 차례로 보강합니다.
        </p>
      </header>
      ...
      <section className="blog-list" aria-label="발행된 블로그 글">
        {posts.length === 0 ? (
          <article className="blog-empty-card">
            <p>아직 공개된 글이 없습니다.</p>
            <span>seo-blog CRM에서 콘텐츠를 생성하면 shared DB를 통해 이곳에 표시됩니다.</span>
          </article>
        ) : (
          posts.map((post) => (
