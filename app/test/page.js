...
// /test — index.html 복사본 + channel.io 디자인 시스템
// 원본: ../../index.html (Visible Dev 보라 톤) / 본 라우트: channel.io 토큰 적용 변형본
...
.mark{width:26px;height:26px;border-radius:var(--r-sm);background:var(--purple);display:grid;place-items:center;color:#fff;font-size:var(--text-base);box-shadow:0 8px 20px rgba(94,86,240,.24)}
...
h1{font-size:var(--text-5xl);line-height:var(--lh-tight);margin:0 0 var(--space-6);font-weight:var(--fw-bold);letter-spacing:var(--ls-tight);max-width:var(--hero-w)}
...
h2{font-size:var(--text-3xl);line-height:var(--lh-snug);margin:0 0 var(--space-4);font-weight:var(--fw-bold);letter-spacing:var(--ls-snug)}
.desc{font-size:var(--text-base);color:#626873;line-height:var(--lh-relaxed);margin:0 auto;max-width:var(--max-copy)}

.flow{max-width:860px;margin:var(--space-16) auto 0;padding:0 18px}
.flow-top,.flow-bottom{height:28px;border-radius:4px;background:linear-gradient(90deg,#eee 0%,#e8e6ff 33%,#d8f2e4 67%,#e2f4ff 100%);position:relative;margin-bottom:var(--space-12)}
.barline{position:absolute;top:-4px;width:4px;height:36px;border-radius:8px;background:#5e56f0}.barline:nth-child(2){left:33%}.barline:nth-child(3){left:66%;background:#31a552}.barline:nth-child(4){right:0;background:#60a5ff}
.sankey{height:190px;position:relative;background:linear-gradient(100deg,#eeeeef 0%,#ece9ff 36%,#d8f4e4 66%,#e2f7ff 100%);clip-path:polygon(0 0,100% 0,100% 66%,72% 66%,72% 100%,40% 78%,0 78%);opacity:.92}
.sankey-label{position:absolute;font-size:var(--text-xs);font-weight:var(--fw-black);color:#30343b}.l1{left:16px;top:72px}.l2{left:320px;top:86px}.l3{left:585px;top:46px}.l4{right:-18px;top:45px}.l5{right:-18px;bottom:38px;color:#9b6129}
...
.feature{padding:var(--section-y-lg) 0}.feature .wrap{max-width:var(--max-feature)}.feature-head{margin-bottom:var(--space-10)}.feature-head .kicker{text-align:left}.feature-head h2{max-width:620px}.feature-head .desc{margin:0;max-width:720px}
...
.final-cta{padding:var(--section-y) 0;text-align:center}.final-cta h2{font-size:var(--text-3xl)}.footer{border-top:1px solid #e8e8ec;padding:var(--space-16) 0 var(--space-24);color:#5f636d}.footer-grid{display:grid;grid-template-columns:1.5fr repeat(5,1fr);gap:var(--space-7)}.footer h4{font-size:var(--text-sm);color:#111;margin:0 0 var(--space-4)}.footer a{display:block;font-size:var(--text-sm);margin:9px 0;color:#737780}

.float-chat{position:fixed;right:28px;bottom:30px;z-index:40;display:flex;flex-direction:column;align-items:flex-end;gap:var(--space-2)}.float-chat .q{background:#fff;border:1px solid #ececf0;border-radius:var(--r-pill);padding:9px 14px;font-size:var(--text-xs);box-shadow:0 8px 22px rgba(0,0,0,.07);color:#555b66}.chat-btn{width:46px;height:46px;border-radius:50%;background:var(--purple);display:grid;place-items:center;color:#fff;font-weight:var(--fw-black);box-shadow:0 10px 26px rgba(94,86,240,.32)}

@media(max-width:900px){.links{display:none}.hero-grid,.feature-grid,.caption-grid,.human-grid,.human-caption,.stats-grid,.platform,.footer-grid{grid-template-columns:1fr}.hero{padding-top:var(--space-12)}.hero-grid{gap:var(--space-8)}h1{font-size:var(--text-4xl)}.hero-visual{min-height:460px}.logo-grid{grid-template-columns:repeat(2,1fr)}.reason-grid{grid-template-columns:1fr 1fr}.app-shot{grid-template-columns:1fr;height:auto}.profile{display:none}.side{display:none}.float-chat .q{display:none}.feature{padding:var(--space-20) 0}.section{padding:var(--space-18) 0}}
`;

const body = `
  <nav class="nav">
    <div class="wrap nav-inner">
      <a class="brand" href="#"><span class="mark">✓</span> Visible Dev</a>
  ...
  <main>
      ...
      <div class="wrap hero-grid">
        <div>
          ...
          <form class="search-cta" id="contact">
            <input placeholder="5분만에 견적을 받아보세요" />
    ...
    <section class="section center">
      <div class="wrap">
        <h2>빠른 외주는 왜<br>항상 퀄리티가 아쉬울까요?</h2>
        <p class="desc">급하게 만들면 매번 즉흥적으로 판단하게 되고, 그때마다 빠뜨리는 항목과 편차가 생깁니다. 그래서 반복되는 판단을 시스템으로 고정해, 빠르게 만들어도 기준이 흔들리지 않게 합니다.</p>
        <div class="flow">
          <div class="flow-top"><span class="barline"></span><span class="barline"></span><span class="barline"></span></div>
          <div class="sankey"><span class="sankey-label l1">100% 요청</span><span class="sankey-label l2">제작 시스템</span><span class="sankey-label l3">검수 12항목</span><span class="sankey-label l4">92% 1차 통과</span><span class="sankey-label l5">8% 보완 후 출시</span></div>
        </div>
      </div>
    </section>
          ...
          <div class="case-track" id="caseTrack">
            <article class="case-card case-1" data-cat="commerce">
              <div class="case-inner">
                <div class="case-brand">러닝 브랜드</div>
                <div class="case-stat">00.0<small>%</small></div>
                <div class="case-stat-label">대표 성과 지표</div>
                <p class="case-quote">“시스템대로 제작·검수해 빠르게 오픈했고, 결과가 좋았습니다.”<em>(실제 고객 인터뷰 문구로 교체 예정)</em></p>
                <div class="case-author">담당자명 · 직함</div>
                <a class="case-more">자세히 보기 ›</a>
              </div>
            </article>
            <article class="case-card case-2" data-cat="homepage">
              <div class="case-inner">
                <div class="case-brand">시공사</div>
                <div class="case-stat">00.0<small>%</small></div>
                <div class="case-stat-label">대표 성과 지표</div>
                <p class="case-quote">“시공 포트폴리오와 문의 흐름을 믿음직하게 정리했습니다.”<em>(실제 고객 인터뷰 문구로 교체 예정)</em></p>
                <div class="case-author">담당자명 · 직함</div>
                <a class="case-more">자세히 보기 ›</a>
              </div>
            </article>
            <article class="case-card case-3" data-cat="platform">
              <div class="case-inner">
                <div class="case-brand">플랫폼</div>
                <div class="case-stat">00.0<small>%</small></div>
                <div class="case-stat-label">대표 성과 지표</div>
                <p class="case-quote">“핵심 화면과 가입·문의 플로우를 빠르게 구현했습니다.”<em>(실제 고객 인터뷰 문구로 교체 예정)</em></p>
                <div class="case-author">담당자명 · 직함</div>
                <a class="case-more">자세히 보기 ›</a>
              </div>
            </article>
    ...
    <section class="feature">
      <div class="wrap">
        <div class="feature-head"><div class="kicker">진행 규칙</div><h2>범위가 부풀지 않게<br>‘필수’와 ‘나중’을 먼저 나눕니다</h2><p class="desc">작업 범위가 부풀어 오르지 않도록, 필수 기능과 추후 개선 항목을 분리합니다. 고객은 지금 어떤 작업이 왜 먼저인지 확인할 수 있습니다.</p></div>
    ...
    <section class="feature">
      <div class="wrap">
        <div class="feature-head"><div class="kicker">상태</div><h2>기록하는 순간<br>외주는 블랙박스가 아니게 됩니다</h2><p class="desc">요청사항과 진행 상태를 기록하면 외주가 블랙박스가 되지 않습니다. 어떤 요청이 완료됐고 어떤 항목이 확인 대기인지 볼 수 있습니다.</p></div>
    ...
    <section class="feature">
      <div class="wrap">
        <div class="feature-head"><div class="kicker">태스크 실행</div><h2>안내로 끝내지 않고<br>실제 결과물까지 만듭니다</h2><p class="desc">상담에서 끝나지 않고 카페24 수정, 랜딩페이지 제작, 자동화 연결까지 필요한 결과물을 만듭니다.</p></div>
    ...
    <section class="section center">
      <div class="wrap">
        <div class="kicker">평가 및 개선</div><h2>공개가 끝이 아니라<br>지표 보고 다음 개선까지</h2><p class="desc">페이지 공개 후 문의, 클릭, 상담 연결 같은 지표를 보며 다음 개선 항목을 정리합니다.</p>
    ...
    <section class="omni center">
      <div class="wrap">
        <div class="kicker">채널박스</div><h2>카톡, 이메일, 게시판, 전화까지<br>프로젝트 채널을 하나로 통합하세요</h2><p class="desc">요청사항이 흩어지지 않도록 한곳에 모으고, 확인해야 할 일을 정리합니다.</p>
    ...
    <section class="final-cta">
      <div class="wrap">
        <h2>지금 만들 사이트만 적어주세요.<br>5분 안에 견적을 드립니다</h2>
  ...
  <footer class="footer">
    <div class="wrap footer-grid">
      <div><a class="brand"><span class="mark">✓</span> Visible Dev</a><p style="font-size:12px;line-height:1.8;margin-top:32px;color:#9a9da5">© 2026 Visible Dev<br/>마감 안에 끝내고, 진행이 보이는 개발 외주</p></div>
  ...
  <div class="float-chat">
    <div class="q">마감일이 언제인가요?</div>
    <div class="q">지금 어디까지 됐나요?</div>
