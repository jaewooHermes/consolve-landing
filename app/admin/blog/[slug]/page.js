  ...
  return (
    <>
      ...
      <main className="wrap">
        ...
        </div><div className="actions"><button className="btn primary" type="submit">DB에 저장</button><button className="btn" type="button" id="syncQuick">빠른 필드 → JSON 반영</button><a className="btn" href="/blog">목록으로</a></div><p className="hint">권장 구조는 content.blocks 배열입니다. HTML은 직접 넣지 말고 paragraph, heading, image, table, list, promptGrid 같은 block type을 사용하세요. 저장 시 이전 payload는 post_revisions에 백업됩니다.</p><div id="statusBox" className="status" role="status" aria-live="polite" /></form></section>
      </main>
      <script dangerouslySetInnerHTML={{ __html: `(() => { const slug=${JSON.stringify(slug)}; const initialAlert=${JSON.stringify(initialAlert)}; const form=document.querySelector('#editForm'); const statusBox=document.querySelector('#statusBox'); const button=form?.querySelector('button[type="submit"]'); const syncButton=document.querySelector('#syncQuick'); const contentJson=document.querySelector('#contentJson'); const setStatus=(type,message)=>{statusBox.className='status '+type;statusBox.textContent=message;}; const applyQuickFields=()=>{const post=JSON.parse(contentJson.value); post.slug=slug; post.title=document.querySelector('#title').value.trim(); post.status=document.querySelector('#status').value; post.category=document.querySelector('#category').value.trim()||'insight'; post.author=document.querySelector('#author').value.trim()||'Consolve'; post.excerpt=document.querySelector('#excerpt').value.trim(); post.lead=document.querySelector('#lead').value.trim()||post.excerpt; post.seoDescription=document.querySelector('#description').value.trim(); if(post.status==='published'&&!post.publishedAt) post.publishedAt=new Date().toISOString(); contentJson.value=JSON.stringify(post,null,2); return post;}; if(initialAlert) alert(initialAlert); if(!form||!statusBox||!button||!contentJson)return; syncButton?.addEventListener('click',()=>{try{applyQuickFields();setStatus('ok','빠른 필드를 JSON에 반영했습니다.');}catch(error){setStatus('err','JSON 형식 오류: '+error.message);}}); form.addEventListener('submit',async(event)=>{event.preventDefault();button.disabled=true;statusBox.className='status';statusBox.textContent='';try{const payload=applyQuickFields();const response=await fetch('/api/admin/blog/'+encodeURIComponent(slug),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payload})});const data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data.error||'저장에 실패했습니다.');setStatus('ok','DB 저장 완료\nslug: '+data.slug+'\nupdatedAt: '+data.updatedAt); alert('DB 저장 완료'); }catch(error){const message=error.message||String(error);setStatus('err',message);alert('저장 실패: '+message);}finally{button.disabled=false;}}); })();` }} />
    </>
  );
}
